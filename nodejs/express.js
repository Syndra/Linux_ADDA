...
function search_op_avg(request, response)
{
  ...
  request.on('end', () =>
  {
    ...
    var connection = mysql_load();
    console.log(data);
    connection.query('SELECT sec_to_time(avg(Alarmoff - Alarmon)) as AVG FROM wakeuplog where (Alarmon > ? or Alarmon < ?) and (Temp > ? or Temp < ?) and (Light > ? or Light < ?) and (Alti > ? or Alti < ?) and (Press > ? or Press < ?)',
